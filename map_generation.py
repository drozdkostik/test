...
import matplotlib.pyplot as plt                           # Імпорт pyplot для візуалізації
...
class MapConfig:                                           # Опис конфігурації карти
    ...
    dpi: int = 1000                                        # Щільність пікселів
...
@dataclass
class CorridorConfig:                                      # Опис конфігурації коридору
...
def rotate_local_to_global(x_local: np.ndarray | float,
                           ...
                           p: EllipseParams) -> Tuple[np.ndarray, np.ndarray]: # Перетворення локальних координат у глобальні
...
def generate_rop_clusters(p: EllipseParams,
                          ...
                          existing_ops: List[Point]) -> Tuple[List[Point], List[Tuple[Point, float]]]: # Генерація РОП-кластерів
...
def generate_op_circles(ops: List[Point], rng: np.random.Generator) -> List[Tuple[Point, float]]: # Генерація кіл навколо ОП
...
def generate_battle_line(cfg: MapConfig, seed: int = 42) -> BattleLine: # Генерація лінії бою
    ...
    """
    num_points = 100                                       # Кількість точок лінії
    ...
    x_end = x_center + 5.0                                 # Кінцевий X
    x_base = np.linspace(x_start, x_end, num_points)       # Базовий масив X

    # Генеруємо шум і згладжуємо його згорткою для плавності
    raw_noise = rng.normal(0, 1.2, size=num_points)        # Випадковий шум
    window_size = 15                                       # Розмір вікна згладжування
